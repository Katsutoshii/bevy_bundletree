...
/// See [`ChildBundles`] for a version that supports adding multiple children of the same type.
///
...
struct ChildBundleCommand<B> {
    parent_entity: Entity,
    _phantom: PhantomData<B>,
}

impl<B: Bundle> Command for ChildBundleCommand<B> {
    fn apply(self, world: &mut World) {
        ...
        let Some(with_child_component) = entity_mut.take::<ChildBundle<B>>() else {
            #[cfg(debug_assertions)]
            panic!("ChildBundle component not found");
...
/// See [`ChildBundle`] for a more convenient API when adding only one child (or multiple children with distinct bundle types).
///
/// Under the hood, this is done using component lifecycle hooks.
///
/// # Examples
...
/// A hook that runs whenever [`ChildBundles`] is added to an entity.
///
    ...
    for ChildBundlesCommand<B, I>
{
    fn apply(self, world: &mut World) {
        ...
        let Some(with_children_component) = entity_mut.take::<ChildBundles<B, I>>() else {
            #[cfg(debug_assertions)]
            panic!("ChildBundle component not found");
...
mod tests {
    ...
    fn with_child() {
        ...
        let parent = world.spawn(ChildBundle((A, B(3)))).id();
        // FIXME: this should not be needed!
        world.flush();

        assert!(!world.entity(parent).contains::<ChildBundle<(A, B)>>());
    ...
    fn with_children_vec() {
        ...
        let parent = world.spawn(ChildBundles(vec![B(0), B(1), B(2)])).id();
        // FIXME: this should not be needed!
        world.flush();

        assert!(!world.entity(parent).contains::<ChildBundles<B, Vec<B>>>());
        assert!(!world.entity(parent).contains::<B>());
    ...
    fn with_child_closure() {
        ...
        let parent = world.spawn(ChildBundles((0..7).map(|i| B(i as u8)))).id();
        // FIXME: this should not be needed!
        world.flush();
    ...
    fn with_distinct_children() {
        ...
        let parent = world.spawn((ChildBundle(A), ChildBundle(B(1)))).id();
        // FIXME: this should not be needed!
        world.flush();
        ...
        let parent = world.spawn((ChildBundle(B(1)), ChildBundle(A))).id();
        // FIXME: this should not be needed!
        world.flush();
    ...
    fn grandchildren() {
        ...
        let parent = world.spawn(ChildBundle((A, ChildBundle((A, B(3)))))).id();
        // FIXME: this should not be needed!
        world.flush();
    ...
    fn hierarchical_bundle() {
        ...
        let parent = world
            .spawn(HierarchicalBundle {
                a: A,
                child: ChildBundle(ABBundle { a: A, b: B(17) }),
            })
            .id();
        ...
        assert!(!world.entity(parent).contains::<ChildBundle<ABBundle>>());
        assert!(world.entity(parent).contains::<A>());
        assert!(!world.entity(parent).contains::<B>());
    ...
    fn command_form() {
        fn spawn_with_child(mut commands: Commands) -> Entity {
            commands.spawn((A, ChildBundle(B(5)))).id()
        }

        let mut world = World::new();
        let parent = world.run_system_once(spawn_with_child).unwrap();

        assert!(!world.entity(parent).contains::<ChildBundle<B>>());
        assert!(world.entity(parent).contains::<A>());
        assert!(!world.entity(parent).contains::<B>());
